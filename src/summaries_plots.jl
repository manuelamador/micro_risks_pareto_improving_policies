...
function summary_statics(
    ...
    con = consumption_alloc(h, equilibrium.r, equilibrium.w, equilibrium.transfer, equilibrium.pol)
    con = reshape(con, 1, :)[1, :]
...
function do_plots(tran, laissez_faire)
    ...
    ylist  = map(k -> output(t; k , n), tran.path.k)
    size = (400, 300)
    y0 = laissez_faire.y
    ...
    push!(flist, plot(100 .* tran.path.b ./ y0; size,
        title = L"$b/y_{0}$", ylabel = L"\%", ylims = (0, 100)))

    ##################
    # K

    push!(flist,
            plot(tran.path.k ./ y0; size, title = L"$k/y_{0}$" , ylims = (0, 1.1 * maximum(tran.path.k ./ y0)))
    )
    ...
    push!(flist,
        begin
            plot(100 .* tran.path.transfer ./ y0; size,
                title = "Transfers and bond revenue", ylabel = L"\%", label = L"$T/y_0$", fill = 0, alpha = 0.2, legend = (.5, .5))
            plot!(100 .* tran.path.transfer ./ y0; color = 1)
            plot!([100 * (bprime - (1 + r) * b) / y0 for (b, bprime, r) in zip(tran.path.b, tran.path.b[2:end], tran.path.r, ylist)];
                size, label = L"$\frac{b' - (1+r)b}{y_0}$")
        end
    )
    ...
    std_con = map(tran.path) do alloc
        con = reshape(consumption_alloc(h, alloc.r, alloc.w, alloc.transfer, alloc.pol), 1, :)[1, :]
        allpdf = reshape(alloc.pdf, 1, :)[1, :]
        100 * StatsBase.std(log.(con), StatsBase.ProbabilityWeights(allpdf))
    end

    con_init = consumption_alloc(h, laissez_faire.r, laissez_faire.w, laissez_faire.transfer, laissez_faire.pol)
    ...
    push!(flist,
        begin
            plot(std_con .- std_con_init; size,
                fill = 0, alpha = 0.2, title = L"$\sigma(\log c) - \sigma(\log c_0)$", ylabel = L"\%")
